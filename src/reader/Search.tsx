...
export function Search({
  ...
  const results = useMemo(
    () => searchDocuments(catalog.documents, deferredQuery),
  ...
  return (
    ...
    >
      ...
      <div className="search-status" aria-live="polite">
        {query.trim()
          ? `${results.length} 篇匹配文档`
          : `全部 ${results.length} 篇文档`}
        <span>↑ ↓ 选择 · Enter 打开</span>
      </div>
      <div className="search-results">
        {results.length === 0 && (
          <p className="empty-message">
            没有找到匹配文档。试试模块名称或更短的关键词。
          </p>
        )}
        {results.slice(0, 80).map((document) => (
          <a
            ...
            onClick={() => dialogRef.current?.close()}
          >
            <strong><Highlight text={document.title} query={deferredQuery} /></strong>
            <code>{document.path}</code>
            <p><Highlight text={excerpt(document, deferredQuery)} query={deferredQuery} /></p>
          </a>
        ))}
        {results.length > 80 && (
          <p className="empty-message">
            已显示前 80 篇，请输入关键词缩小范围。
          </p>
        )}
      </div>
    </dialog>
