  ...
  return (
    <>
      ...
      <header className="topbar">
        ...
        <button className="theme-toggle" onClick={toggle} aria-label={theme === 'dark' ? '切换为浅色主题' : '切换为深色主题'} title={theme === 'dark' ? '切换为浅色主题' : '切换为深色主题'}>
          {theme === 'dark' ? '☀' : '☾'}
        </button>
        <button
...
<p>{catalog.documents.length ? '文件可能已移动，试试搜索文档标题或回到文档首页。' : '在项目目录中创建 README.md 或其他 .md 文件，然后刷新预览。'}</p>
              <a href="#/">返回文档首页 →</a>
            </section>
          )}
        </main>
