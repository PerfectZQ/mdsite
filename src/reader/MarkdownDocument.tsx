...
export function MarkdownDocument({
  ...
  useEffect(() => {
    ...
    mermaid?.initialize({ startOnLoad: false, securityLevel: 'strict', suppressErrorRendering: true, theme: theme === 'dark' ? 'dark' : 'default', fontFamily: 'system-ui, sans-serif' })
    let cancelled = false
    const sourceBlocks = [
      ...element.querySelectorAll<HTMLElement>('pre > code.language-mermaid'),
    ]
    const render = async () => {
      for (const code of sourceBlocks) {
        ...
        try {
          ...
          const { svg } = await mermaid.render(
            `documentation-diagram-${++diagramID}`,
            code.textContent || '',
          )
          if (cancelled) return
      ...
      if (!cancelled && latestHeading.current)
        document
          .getElementById(latestHeading.current)
  ...
  return (
    <>
        ...
        onClick={(event) => {
          ...
          if (target?.dataset.sourcePath) {
            ...
            onSourceLink(
              ...
              target.dataset.directory === 'true',
            )
          } else if (target?.hash === window.location.hash) {
            const heading = new URLSearchParams(target.hash.split('?')[1]).get(
              'heading',
            )
            if (heading)
              document
      ...
      <footer className="document-footer">
        ...
        <a
          href={documentHref(current.path)}
