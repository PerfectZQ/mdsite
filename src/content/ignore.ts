...
const defaults = ['.git', '.hg', '.svn', '.codegraph', 'node_modules', 'vendor', 'dist', 'output', '.next', '.venv', 'venv', '__pycache__']
...
export async function readIgnore(root: string, exclude: string[] = []) {
  ...
  const excluded = exclude.map(path => path.trim().replaceAll('\\', '/').replace(/^\.\//, '').replace(/^\/+|\/+$/g, '')).filter(Boolean)
