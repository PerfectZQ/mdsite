...
function localURL(raw: string, from: string) {
  ...
  try {
    ...
    const name = (path ? (path.startsWith('/') ? posix.normalize(path.slice(1)) : posix.join(posix.dirname(from), path)) : from).replace(/\/$/, '') || '.'
...
export async function renderMarkdown(root: string, path: string, source: string, known: Set<string>, aliases: Record<string, string>, ignored: Ignore): Promise<DocumentRecord> {
  const raw = markdown.render(source.replace(/^---\r?\n[\s\S]*?\r?\n---(?:\r?\n|$)/, ''))
  const document = parseFragment(sanitize(raw, {
    allowedTags: [...sanitize.defaults.allowedTags, 'img', 'input', 'details', 'summary', 'del'],
    allowedAttributes: {
      '*': ['id', 'class'], a: ['href', 'title'], img: ['src', 'alt', 'title', 'width', 'height'],
      input: ['type', 'checked', 'disabled'], th: ['align', 'colspan', 'rowspan'], td: ['align', 'colspan', 'rowspan'],
      ol: ['start'],
    },
  ...
  async function visit(node: Node) {
    if ('tagName' in node) {
      if (/^h[1-6]$/.test(node.tagName)) {
        ...
        const base = title.toLowerCase().replace(/\s/gu, '-').replace(/[^\p{L}\p{N}_-]/gu, '') || 'section'
        let id = base
      ...
      if (node.tagName === 'a') {
        ...
        if (href && target) {
          ...
          if (posix.extname(target.name).toLowerCase() === '.html') candidates.push(target.name.replace(/\.html$/i, '.md'))
          candidates.push(...[...known].filter(name => posix.dirname(name) === target.name && posix.basename(name).toLowerCase() === 'readme.md'))
          const found = candidates.map(name => aliases[name] ?? name).find(name => known.has(name))
          if (found) {
            attribute(node, 'href', `#/${found.split('/').map(encodeURIComponent).join('/')}${target.heading ? `?heading=${encodeURIComponent(target.heading)}` : ''}`)
          } else {
        ...
        if (target) {
          ...
          if (src && mime && !ignored(target.name)) {
            try {
              ...
              if (!ignored(resolved)) attribute(node, 'src', `data:${mime};base64,${(await readFile(join(root, resolved))).toString('base64')}`)
            } catch {
  ...
  return {
    path, source, html: serialize(document), headings, text,
    title: headings[0]?.title || posix.basename(path, posix.extname(path)),
    summary: [...summary].slice(0, 150).join(''), words: [...text].length,
  }
}
