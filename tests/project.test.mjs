...
test('project files become a searchable offline page with native paths and Markdown features', async t => {
    ...
    assert.deepEqual(
        catalog.documents.map(doc => doc.path),
        ['.feature/plan.md', 'README.md', 'docs/guide.md', 'internal/mod/readme.MD'],
    )
    assert.equal(catalog.title, '示例项目')
    assert.equal(catalog.home, 'README.md')
    const home = catalog.documents[1].html
    for (const fragment of ['#/docs/guide.md?heading=', '#/internal/mod/readme.MD', '#/docs/guide.md'])
        assert.ok(home.includes(fragment), fragment)
    ...
    for (const fragment of [
        ...
        assert.ok(guide.html.includes(fragment), fragment)
...
test(
    'file aliases deduplicate search; directory links and outside links are skipped',
    ...
    async t => {
        ...
        for (const [name, target] of Object.entries({
            'AGENTS.md': 'README.md',
            linked: 'docs',
            'directory-link.md': 'linked/guide.md',
...
test('root ignore rules refresh, support exceptions, and cannot override explicit exclusions', async t => {
    const root = await fixture(t, {
        'README.md': '# Project',
        'generated/README.md': '# Kept',
        'generated/api.md': 'EXCLUDED_GENERATED_BODY',
        'docs/guide.md': '# Guide',
        'docs/.mdignore': 'guide.md',
        '.gitignore': 'docs/',
        'dist/README.md': '# Reincluded default',
        'private/note.md': 'EXCLUDED_PRIVATE_BODY',
        '.mdignore': 'generated/**\n!generated/README.md\n!dist/\n!private/\n',
    })
...
test('ignore rules also protect images and symlink targets', {skip: process.platform === 'win32'}, async t => {
    const root = await fixture(t, {
        ...
        'private/note.md': 'EXCLUDED_LINK_BODY',
        'private/picture.svg': '<svg xmlns="http://www.w3.org/2000/svg"/>',
    })
...
test('preview refreshes files and ignore rules, and serves only the generated UI', async t => {
    ...
    assert.deepEqual(
        catalog.documents.map(doc => doc.title),
        ['After'],
    )
...
test('CLI builds outside its package, validates arguments, and preserves previous output on failure', async t => {
    ...
    for (const args of [
        ['build', '.', '-o', 'README.md'],
        ['build', 'missing', '-o', 'existing.html'],
        ['build', '.', 'extra'],
        ['unknown'],
        ['serve', '--addr', 'invalid'],
        ['build', '--addr', '127.0.0.1:0'],
    ]) {
...
test('CLI refreshes changed resources and preserves unchanged output timestamps', async t => {
    ...
    assert.ok(
        (await documents())[0].html.includes(Buffer.from('<svg><title>Updated image</title></svg>').toString('base64')),
    )
    await writeFile(join(root, 'README.md'), '# After')
    await writeFile(join(root, 'extra.md'), '# Extra')
    await run()
    assert.deepEqual(
        (await documents()).map(doc => doc.title),
        ['After', 'Extra'],
    )
    await rename(join(root, 'extra.md'), join(root, 'moved.md'))
    await run()
    assert.deepEqual(
        (await documents()).map(doc => doc.path),
        ['README.md', 'moved.md'],
    )
...
test('ignore matching agrees with Git across patterns and directory pruning', async t => {
    ...
    const names = [
        ...
        'generated/README.md',
        'generated/a.md',
        'generated/nested/a.md',
        'file.md',
        'nested/file.md/README.md',
        '#note.md',
        '!note.md',
        '[note].md',
        'space .md',
    ...
    for (const pattern of patterns) {
        ...
        const expected = result.stdout
            ...
            .sort()
        assert.deepEqual(
            (await scan(root)).documents.map(doc => doc.path),
            expected,
            pattern,
        )
    }
